...
|----|------|-------------|-------------|
| R-01 | **Owner (Admin)** | Agent builder who controls the platform | Full access. Provisions user tokens, enables/disables accounts via Supabase DB. Creates group conversations. |
| R-02 | **Tester** | Invited friend/colleague | Send/receive messages in DMs and groups. View online presence. Upload files. Cannot manage users. |
| R-03 | **Agent** | AI agent integrated via Supabase API | Send/receive messages (text, URLs, docs, images) in assigned DMs or groups. No UI access — API only. |
...
| FR-18 | Message reactions | P3 | React to messages with emoji. One reaction per user per message. Show reaction counts. | S-03, S-04 |
...
| S-02 | Main Layout (Sidebar) | Left sidebar with presence, conversation list, search. Right area for active chat. | P1 |
| S-03 | DM Chat | 1:1 message thread with input, attachments, markdown rendering | P1 |
| S-04 | Group Chat | Multi-party message thread with member list, agent badges | P2 |
...
|----|--------|-------------|------------|
| E-01 | `users` | Human users and agents | `id` (uuid, PK), `email` (text, unique), `display_name` (text), `avatar_url` (text, nullable), `is_agent` (bool, default false), `is_active` (bool, default true), `token` (text, unique), `last_seen_at` (timestamptz) |
