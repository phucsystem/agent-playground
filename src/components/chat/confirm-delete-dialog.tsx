...
export function ConfirmDeleteDialog({ open, onClose, onConfirm }: ConfirmDeleteDialogProps) {
  ...
  return (
    <dialog
      ...
      className="fixed inset-0 z-50 bg-transparent backdrop:bg-black/40 p-0 m-auto rounded-2xl"
    >
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm mx-4 animate-in fade-in zoom-in-95 duration-150">
        <div className="flex items-center gap-3 mb-3">
          <div className="flex items-center justify-center w-10 h-10 rounded-full bg-red-100">
            <Trash2 className="w-5 h-5 text-red-600" />
          </div>
          <h3 id="confirm-delete-title" className="text-lg font-semibold text-neutral-900">Delete message</h3>
        </div>
        <p id="confirm-delete-desc" className="text-sm text-neutral-500 mb-5">
